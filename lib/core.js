...
/*
  For precompiled expressions attrs and pcex are arrays

  riot.tag2 = function (name, html, css, attrs, fn, pce) {
    if (css) injectStyle(css)
    if (pce) tmpl.insert(pce)
 ...
 * @param {Object} opts - User defined options, including teh brackets object.
 * @returns {string} Formated attributes, with whitespace converted to compact spaces.
 */
function parseAttrs(str, opts) {
  var
...
// 4. replace ':selector' with tag or prepend tag, and mirror to [riot-tag]
...
/*
  ...
  In tag files, custom tags can span multiple lines, but no other external element
...
//#endif
  ...
  return src
    ...
    .replace(CUST_TAG, function (_, tagName, attribs, content) {
      ...
      if (attribs)
        attribs = parseAttrs(parseHtml(attribs, opts, pcex), opts)
