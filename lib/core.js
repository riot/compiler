...
var
  ...
  */
  ...
  E_NUMBER = regEx.E_NUMBER,
  PCE_TEST = regEx.PCE_TEST,
...
function mktag(name, html, css, attrs, js, pcex) {
  ...
  // adds the object with precompiled expressions, if any
  if (pcex.length) s += '}, {' + pcex.join(c)
  ...
  while (match = HTML_ATTR.exec(str)) {
    ...
    if (!v)
      ...
      if (k === 'type' && v.toLowerCase() === '"number"') {
        v = '"' + E_NUMBER + '"'  // fix #827 by @rsbondi
      }
      else if (PCE_TEST.test(v)) {
...
function splitHtml(html, opts, pcex) {
  ...
  // `tmpl.compile` is a heavy function, so don't call it if not necessary
  if (html && ~html.indexOf(opts._b[0])) {
    var
      fn = opts.expr && (opts.parser || opts.type) ? compileJS : 0
    html = tmpl.compile(html, opts, pcex, fn)
  }
  return html
}
...
function compile(src, opts, url) {
  ...
  // get a static brackets array for use in the entire source
  opts._b = brackets.array(opts.brackets)
  ...
  return src
    ...
    .replace(CUST_TAG, function (_, tagName, attribs, content) {
      ...
      if (attribs)
        attribs = parseAttrs(splitHtml(attribs, opts, pcex))
