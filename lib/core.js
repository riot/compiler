...
function riotjs (js) {
  ...
  while (rm = R.exec(js)) {
    if (rm[0][0] === '/' && !rm[1] && !rm[2]) { // $1:div, $2:regex
      ...
      R.lastIndex = rm[3] + 1          // $3:matchOffset
    }
  }
 ...
 * Runs any parser for style blocks and call scopedCSS if required.
 *
...
/**
 * Public API to the compileCSS function, makes type and opts optional.
 *
 * @param   {string} css    - Raw style block
 * @param   {string} [type] - Must be one of `parsers.css`
 * @param   {object} [opts] - User options passed to the given parser
...
var
   ...
   * This regex does not expect expressions, so do not handles escaped quotes.
   * @const {RegExp}
   */
  TYPE_ATTR = /\stype\s*=\s*(?:(['"])(.+?)\1|(\S+))/i,
  /**
   * Source string for construct generic regexes matching pairs of `attribute=value`.
   * Used for `option`, `src`, and `charset`, can handle escaped quotes and
   * unquoted JSON objects without nested brackets.
   * @const {string}
   */
 ...
 *
 * @param   {string} attrs - The attribute list
 ...
 *
 * @param   {string} code  - Unprocessed code
 * @param   {object} opts  - Compiler options
...
/**
 * Runs the custom or default parser on the received CSS code.
 *
...
/**
 * Matches HTML tag ending a line. a) self-closing tag, b) closing tag or tag without
 * attributes, c) void tag. This regex still can be fooled by strange code like:
 * ```js
 * x <y -y>
 *  z
 * ```
...
/**
 * Separate the HTML and JS parts of the tag.
 * The last HTML element (can be a `<script>` block) must terminate its line.
 *
 * @param   {string} str - Tag content, normalized, without attributes
 * @returns {Array} Parts 0:HTML, 1:JavaScript
 */
...
var
   ...
   * the same indentation (size/type).
   * CUST_TAG recognize escaped quotes to allow nested quotes in JS string inside unquoted
   * expressions, but do not allow unquoted expressions containing the `>` operator.
   * @const {RegExp}
   */
  CUST_TAG = _regEx(/^([ \t]*)<([-\w]+)(?:\s+([^'"\/>]+(?:(?:@|\/[^>])[^'"\/>]*)*)|\s*)?(?:\/>|>[ \t]*\n?([\S\s]*)^\1<\/\2\s*>|>(.*)<\/\2\s*>)/
    .source.replace('@', S_STRINGS), 'gim'),
  /**
   * SCRIPTS and STYLES disallows the operator `>` in attribute values.
   */
  SCRIPTS = /<script(\s+[^>]*)?>\n?([\S\s]*?)<\/script\s*>/gi,
  STYLES = /<style(\s+[^>]*)?>\n?([\S\s]*?)<\/style\s*>/gi
