...
export default async function css(sourceNode, source, options, { code, map }) {
  ...
  const preprocessorOutput = await (preprocessorType ?
  ...
  return mergeOutputChunks({ code, map }, output)
}
...
function generateSourcemap(source, cssNode, file, { line, column }) {
  ...
  splitStringByEOL(cssNode.text).forEach(row => {
    ...
    sourcemap.addMapping({
      ...
      original: getLineAndColumnByPosition(source, pointer.originalPosition + charsLength)
    })

    pointer.generatedPosition += charsLength
  })
